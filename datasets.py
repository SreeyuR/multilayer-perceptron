...
class Dataset:
    def __init__(self, dist0, dist1):
        ...
        dataset_points[0 : self.N, -1] = np.ones(
            (1, self.N)
        )  # set N points to 1 (M points to 0)
    ...
    def save_dataset_plot(self):
        os.makedirs("figs/datasets", exist_ok=True)
        self.show_dataset(save=True)
        print("Dataset plot has been saved in figs/datasets")
    ...
    def show_dataset(self, save=False):
        ...
        if self.dataset_type == kIris:
            ...
            plt.axis("equal")
            if save:
                plt.savefig("figs/datasets/{}.png".format(self.dataset_type))
            else:
                plt.show()
...
class GaussianDistribution(Distribution):
    def __init__(self, n, u, v, dims):
        """
        Input
            n : number of points
            u : a specified mean
            v: a specified variance
            dims: the dimensionality of the dataset
        Returns:
            A gaussian-distributed dataset with specified mean and
            variance of a specified dimensionality with n points
        """
        global generator
        super().__init__(n, dims)

        mu = np.ones(dims) * u
        d = generator.randint(v, size=(1, dims))[0]
        cov = np.diag(d)
...
def generate_nd_dataset(N, M, distribution, dims=2):
    ...
    if distribution == kGaussian:
        ...
        u0 = 10  # mean of the first gaussian
        v0 = 25  # variance of the first gaussian
        gaus0 = GaussianDistribution(N, u0, v0, dims)
        u1 = 50  # mean of the second gaussian
        v1 = 25  # variance of the second gaussian
        gaus1 = GaussianDistribution(M, u1, v1, dims)
        dataset = Dataset(gaus0, gaus1)

    elif distribution == kCircles:
        # circles is only 2d
        circle0 = CircleDistribution(N, 2.0)
        circle1 = CircleDistribution(M, 0.5)
...
def train_test_split_(dataset):
    #
    # Input: the dataset output from generate_nd_dataset
    #
